...
class LaneDetectionTester:
    ...
    def process_frame(self, frame):
        ...
        # Step 5: Make navigation decisions (for images, we always make decision)
...
def get_image_file():
    """
    Get image file path from user input or command line argument
    """
    if len(sys.argv) > 1:
...
def main():
    ...
    print("Angle-Based Lane Detection System - Image Mode")
    print("=" * 60)
    ...
    print("- Stable line detection")
    print("=" * 60)
    print("Controls:")
    print("- Press 'q' to quit")
    print("- Press 's' to save processed image")
    print("=" * 60)
    ...
    # Add processing time info to image
    if processing_time > 0.001:
        fps_text = f"Processing time: {processing_time*1000:.1f}ms"
    else:
        fps_text = f"Processing time: <1ms"
    cv2.putText(processed_frame, fps_text, (processed_frame.shape[1] - 250, 20), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
    
    # Print results to console
    print(f"Processing completed in {processing_time*1000:.2f}ms")
    print(f"Direction: {nav_cmd['direction']}")
    print(f"Left Motor Speed: {nav_cmd['left_speed']}")
    print(f"Right Motor Speed: {nav_cmd['right_speed']}")
    print(f"Confidence: {nav_cmd['confidence']}")
    print(f"Left Angle: {nav_cmd['left_angle']:.1f}°")
    print(f"Right Angle: {nav_cmd['right_angle']:.1f}°")
    print(f"Angle Difference: {nav_cmd.get('angle_difference', 0):.1f}°")
    print("=" * 60)
    
    # Display the processed image
    cv2.imshow('Image Lane Detection', processed_frame)
    ...
    while True:
        ...
        if key == ord('q'):
            ...
            output_filename = f"{base_name}_lane_detected.jpg"
            cv2.imwrite(output_filename, processed_frame)
            print(f"Processed image saved as: {output_filename}")
    
    cv2.destroyAllWindows()
    print(f"Image lane detection completed for: {os.path.basename(image_path)}")
