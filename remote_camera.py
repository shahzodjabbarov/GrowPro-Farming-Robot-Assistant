...
def main():
    ...
    print("Angle-Based Lane Detection System")
    print("=" * 50)
    print("Features:")
    ...
    while True:
        ...
        if current_time - last_print_time >= 0.5:
            frame_count += 1
            print(f"Frame {frame_count:3d} | Dir: {nav_cmd['direction']:12} | "
                  f"L: {nav_cmd['left_speed']:3d} | R: {nav_cmd['right_speed']:3d} | "
                  f"Conf: {nav_cmd['confidence']:2d} | "
                  f"L_Angle: {nav_cmd['left_angle']:6.1f}° | "
                  f"R_Angle: {nav_cmd['right_angle']:6.1f}° | "
                  f"Diff: {nav_cmd.get('angle_difference', 0):6.1f}°")
            last_print_time = current_time
        
        cv2.imshow('Angle-Based Lane Detection', processed_frame)
        
        key = cv2.waitKey(1) & 0xFF
        if key == ord('q'):
            break
        elif key == ord('s'):
            cv2.imwrite(f"angle_lane_frame_{frame_count}.jpg", processed_frame)
            print(f"Saved frame {frame_count}")
    
    cap.release()
    cv2.destroyAllWindows()
    print("Angle-based lane detection completed.")
